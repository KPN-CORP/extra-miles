...
./vendor/bin/phpunit  # tests (`php artisan test` is not available: Collision isn't installed)
./vendor/bin/phpunit --filter=DarwinboxClientTest   # single test / single method
```
...
1. **Admin (session)** — SSO only. `SsoController` receives a base64+XOR-obfuscated payload (email + token) from Darwinbox; `App\Services\DarwinboxClient` verifies the token against `checkToken` **and** requires the response to name the same email before `Auth::login()`ing the matching `User` — the payload email alone is caller-controlled and must never be trusted. All Darwinbox credentials and vendor JWT secrets come from `config/services.php` (`darwinbox`, `sso_vendors`) via `.env`. There are four entry points (`dbauth`, `dbauthlms`, `dbauthcmpr`, `dbauthexpl`) differing only in the `system` session value and redirect target. `GET /admin/login` just bounces back to Darwinbox — there is no local login form.
...
**Page chrome.** Every screen is built from `components/Layout/`: `AppShell` (page background, content column, safe-area padding, route transition, and — with `nav` — the `AppNav` navigation and the padding that clears it), `AppHeader` (sticky top bar with back button, title, and a `trailing` slot), plus `SectionHeader` and `EmptyState`. Tab destinations (`/`, `/event`, `/news`, `/wellness`, `/survey`) render `<AppShell nav>`; drill-down pages get `AppHeader` without navigation. Every page goes through `AppShell` — the two login screens (`ConfirmLogin`, `LoginFailed`) are the only exceptions, and they are centred single messages with no column or nav. Do not hand-roll a back button or a page background — the `100dvh` height, `env(safe-area-inset-*)` and the `.tap` press feedback all live in `resources/css/global.css` behind `app-surface` / `app-bg` / `pb-nav` / `tap`.

**Four layout states.** The app is not phone-only. Two custom Tailwind screens in `tailwind.config.js` decide everything: `short` (`max-height: 500px`, a phone on its side), `wide` (`min-width: 1024px`, tablet landscape), and `rail` — either of those. The rule is *rail when wide or short*: `AppNav` is a bottom tab bar on phone portrait and tablet portrait, and a left rail in the other two states, because that is exactly when vertical space is scarce or horizontal space is plentiful. It is one component whose shape is decided by CSS, not by JavaScript, so rotating a device changes it without a re-render. `--app-column` (480 / 640 / 760 / 900), `--app-header-h` and `--app-rail-w` carry the per-state measurements; pages read them through `.app-container` and never branch on device themselves. List pages go two-up at `rail:`, detail pages split into a 320px media/meta column plus content. `--app-safe-left` / `--app-safe-right` matter in landscape, where the notch moves to the side the rail stands on; the rail offset is opt-in via `.app-has-nav` so pages without navigation don't get an empty gutter. There is deliberately **no** device-detection library and no orientation lock: `screen.orientation.lock()` does nothing on iOS Safari, so every orientation gets a real layout instead. The gate in `app.jsx` that decides mobile-vs-desktop tests `(pointer: coarse)` / `(hover: none)` — capability, not resolution, because an iPad reports a macOS User-Agent and any width test breaks when a phone is turned sideways.

Brand red is the `brand-*` scale in `tailwind.config.js` (`brand-700` is the old `red-700`); older files still say `red-700`.
...
- Attendance is a QR check-in, and the QR belongs to the **activity type**, not the session: each type generates a `qr_token` UUID on create, admins can rotate it (`/admin/wellness/types/{id}/qr`), and the SPA scans it via `WellnessQrScannerModal`. One printed code is posted wherever that type happens and covers every session under it, so the scan has to work out which session it means — `WellnessRegistrationService::checkIn()` takes the employee's registrations under that type, keeps the ones whose check-in window contains the moment of the scan, and marks the *earliest* one not already attended. That is what makes back-to-back sessions work: 08:00–10:00 takes the 09:50 scan, and a second scan at 10:05 falls through to the 10:00–12:00 session. The window is per type — `check_in_opens_minutes_before` / `check_in_closes_minutes_after`, null on the row meaning "follow `config/wellness.php`".
